...
export const transactionsApi = {
  getAll: (params?: { budget_period_id?: number; current_date?: string; search?: string; start_date?: string; end_date?: string; transaction_type?: string[]; category_id?: number[]; currency?: string[]; amount_gte?: number; amount_lte?: number; ordering?: '-date' | 'date' | '-description' | 'description' | '-amount' | 'amount' | '-type' | 'type' | '-category__name' | 'category__name' | '-currency__symbol' | 'currency__symbol'; page?: number; page_size?: number }) => api.get('/transactions', { params }),
